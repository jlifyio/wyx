...
**`/wyx:audit`** — Discovery-only project scanner (~114 lines, read-only). Scans for spec coverage gaps, detects pipeline/sync candidates via code pattern analysis, and outputs a dependency-ordered TODO list of individual skill commands. Uses Glob+Grep directly (no subagents). Evaluates directories for behavioral cohesion before flagging — type definitions, stateless utilities, thin store wrappers, and schema-only directories are filtered. Does not generate specs or modify files.
