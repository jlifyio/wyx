...
**Deliverables**: `.claude-plugin/plugin.json` + `.claude-plugin/marketplace.json` + `hooks/hooks.json` + `scripts/` + 4 SKILL.md files in `skills/`.
...
**Version**: Update in both `.claude-plugin/plugin.json` and `.claude-plugin/marketplace.json` (2 locations: `metadata.version` + `plugins[0].version`).
