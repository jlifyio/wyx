...
description: >
  ...
  dependencies", "wyx map", or wants a Mermaid graph with dependency
...
- **No nested subgraphs** — Mermaid has open rendering bugs with nesting (label cutoff, direction inheritance). Use the `:::infra` classDef for infrastructure concepts instead.
- **No linking TO subgraph names** — link to nodes inside subgraphs for predictable edge routing. Never write `Node -.-> layerName` to summarise a fan-out; emit each fan-out edge to the individual nodes. The full relationship is captured losslessly per-node in the dependency matrix (Output Stability Rule 8), never relocated out of the graph. A dense node (e.g. a store every concept persists to) keeps all its individual edges.
- **Visual density is unsolved by design** — a large, highly-connected project renders dense, and no deterministic classification-free rule thins it: any inferred infra/domain grouping coin-flips run-to-run and churns `git diff`. Determinism and a trustworthy `git diff` are prioritised over rendered density. The readability escape hatch is **scoped mode** (`/wyx:map src/path/`), which maps one subsystem at a time without thinning anything.
