...
- Spec silence is not drift. Do not infer a contradiction from what a spec leaves unsaid: a behavior the spec does not mention is *undocumented*, not *contradicted*, and a narrow spec statement does not imply the negation of everything it omits (e.g. "field X is not filterable" does not assert "X is not returned"). Flag a contradiction only against an explicit spec statement. This does not relax the Missing action / New state / New dependency / Missing stage checks, which deliberately surface specific *undocumented additions* — those remain in force.
...
- Cross-concept access that the project's `CLAUDE.md` / `AGENTS.md` *explicitly* documents as an intentional architectural convention (e.g. "route load functions may read scalar fields across concepts") is Low severity, not Critical — but the finding MUST recommend formalizing it into the source concept's `## known coupling` so the sanction lives in the spec, not only in project prose. The demotion requires an *explicit* documented convention, never an inferred or implied one; when in doubt, the access stays Critical. Such findings are reported as Low, never silently suppressed.
...
**Clearing the post-fix re-scan reminder (optional).** A fix pass edits the specs, so their mtime becomes newer than the originating `detect` entry — at the next session start the hook will again flag the specs as modified since the last drift check. This is expected: a spec edit is exactly what that reminder tracks. To clear it honestly, re-run `/wyx:concept drift` once — a real scan of the now-clean specs writes a fresh `detect` entry whose timestamp post-dates the fixes, so the specs are no longer newer than the last measurement. Do **not** hand-write a `detect` entry with `specs_with_drift: 0` from memory: a `detect` entry asserts an actual measurement, and forging a clean one silences future warnings against unverified specs (the stale-spec failure mode). Re-measure, don't synthesize.
