...
description: >
  ...
  "design a new module", "wyx concept", or wants to generate, update,
  or drift-check CONCEPT.md files. Supports retrofit, greenfield, drift
  detection, and discovery modes.
...
3. Map existing functions/methods to actions. **Skip orchestration-only functions** — if a function's body only calls other concepts' actions in sequence (e.g., `A.step1() → B.step2() → C.step3()`) without owning state updates of its own, it is orchestration, not a concept action. Orchestration belongs to the caller, a parent concept, or a SYNCS.md handler. Declaring it here creates drift when implementations inevitably split into separate calls.
4. Identify state (what data does this module own?):
   - Check whether any state overlaps with state already declared in other CONCEPT.md files — flag overlaps as potential Rule 3 violations.
   - If the directory contains a mix of concept-shaped files (with state ownership and lifecycle) and infrastructure files (stateless utilities, type definitions, schemas), spec only the concept-shaped subset and list infrastructure as `## dependencies`, not as concept subjects.
   - If naming a state field with another concept's name (e.g., `user: User`), confirm a same-named implementation class actually exists; otherwise describe the state structurally (e.g., `user: { id, role, ... }`). Concept names are logical boundaries, not required class names — using them as types without a matching class guarantees drift.
